...
const logger             = require("./logger");const {
  ADMIN_PASSWORD, ADMIN_TOKEN,
  CLOSE_CODES
} = require("./config");
const {
  roomsMeta, liveRooms, findRoom, ensureLiveRoom, banIp
} = require("./roomStore");
...
router.post("/admin/remove-room", (req, res) =>
  collectJSON(req, res, ({ hash }) => {
    const room = liveRooms.get(hash);
    if (room) {
      room.activeClients.forEach(set =>
        set.forEach(sock =>
          sock.close(CLOSE_CODES.ROOM_DELETED, "Room deleted")));
...
router.post("/admin/update-blocklist", (req, res) =>
  collectJSON(req, res, ({ hash, list }) => {
