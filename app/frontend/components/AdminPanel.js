...
/* ---------- Template laden ----------------------------- */
async function loadTemplate(base) {
  const res = await fetch(`${base}/components/admin-template.html`);
...
export default class ChatAdmin extends HTMLElement {
  ...
  async connectedCallback() {
    ...
    const tplHtml = await loadTemplate(HTTP_BASE);
    ...
    if (localStorage.getItem("adminToken")) this.#showPanel();
  }
  ...
  /* ------------ Panel‑Init ----------------------------- */
  #showPanel() {
    if (this.#panelInitialized) return;  // schon da → nichts tun
    this.#panelInitialized = true;       // nächstes Mal abbrechen
  ...
  /* ------------ Räume ---------------------------------- */
  async #fetchRooms() {
    try { this.state.rooms = await API.getRooms(); }
    catch (e) { console.error("Fehler beim Laden der Räume:", e); }
  }
  ...
  async #createRoom() {
    ...
    try {
      const { hash } = await API.addRoom(name);
      ...
      if (newRoom) {
        ...
        this.shadowRoot.querySelectorAll(".room-title")
            .forEach(el => el.classList.toggle(
              "selected", el.textContent.includes(newRoom.name)));
      }
    } catch (e) {
  ...
  async #selectRoom(hash, name) {
    ...
    try {
      this.state.blocklist = await API.getBlocklist(hash); // ← Array!
    } catch (e) {
    ...
    bus.emit("room-selected", payload);            // ← neu
  }
  ...
  async #updateBlocklist(list) {
    ...
    try {
      await API.updateBlocklist(this.#currentHash, list);
  ...
  #btnCopy(text) { return this.#btn("⧉","copy",() => navigator.clipboard.writeText(text)); }
  #span(c,t) { return Object.assign(document.createElement("span"),{className:c,textContent:t}); }
  ...
  #renderRooms() {
    ...
    this.state.rooms.forEach(({hash,name}) => {
      ...
      wrap.append(a,this.#span("hash",hash)); li.append(wrap);
      li.append(this.#btn("✖","remove",()=>this.#removeRoom(hash)));
  ...
  #renderBlocklist() {
    ...
    if (!this.state.blocklist.length) {
      this.$blocks.innerHTML="<li>Keine IPs blockiert</li>"; return;
    }
    this.state.blocklist.forEach(ip=>{
      const li = document.createElement("li");
      li.append(this.#span("ip-entry",ip),
                this.#btnCopy(ip),
